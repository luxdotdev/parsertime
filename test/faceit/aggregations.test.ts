...
function matchRow(p: Partial<FaceitTeamMatchRow> & { won: boolean }): FaceitTeamMatchRow {
  return { matchId: Math.random().toString(), finishedAt: D("2026-01-01"), tier: FaceitTier.OPEN, ...p };
}

function mapRow(p: Partial<FaceitTeamMapRow> & { won: boolean }): FaceitTeamMapRow {
  return {
    matchId: "m",
...
describe("faceit aggregations", () => {
  ...
  it("computes per-map and per-type winrates with a min-sample gate", () => {
    const rows: FaceitTeamMapRow[] = [
      ...Array.from({ length: 4 }, () => mapRow({ won: true, mapName: "Ilios", mapType: "Control" })),
  ...
  it("computes hero-ban-environment winrate split and delta with min-sample gate", () => {
    const rows: FaceitTeamMapRow[] = [
      ...Array.from({ length: 4 }, (_, i) => mapRow({ won: i === 0, heroBans: ["Sombra"] })),
      ...Array.from({ length: 4 }, () => mapRow({ won: true, heroBans: ["Tracer"] })),
    ];
  ...
  it("aggregates roster strength as the mean of rated starters", () => {
    const roster: FaceitRosterPlayer[] = [
      { faceitPlayerId: "a", nickname: "a", battletag: null, role: "TANK", appearances: 10, appearanceShare: 1, starter: true, fsr: 3000, tsr: 3200 },
      { faceitPlayerId: "b", nickname: "b", battletag: null, role: "DAMAGE", appearances: 9, appearanceShare: 0.9, starter: true, fsr: 4000, tsr: null },
      { faceitPlayerId: "c", nickname: "c", battletag: null, role: "SUPPORT", appearances: 1, appearanceShare: 0.1, starter: false, fsr: 9999, tsr: 9999 },
    ];
  ...
  it("builds map and hero-ban recommendations from rated entries", () => {
    const recs = buildRecommendations({
      byMap: [
        { key: "Ilios", played: 10, won: 8, winRate: 80, weightedWinRate: 80, rated: true },
        { key: "Nepal", played: 10, won: 3, winRate: 30, weightedWinRate: 30, rated: true },
        { key: "Oasis", played: 1, won: 0, winRate: 0, weightedWinRate: 0, rated: false },
      ],
      heroBanEnvironment: [
        { hero: "Sombra", bannedPlayed: 5, bannedWon: 1, bannedWinRate: 20, notBannedPlayed: 5, notBannedWon: 4, notBannedWinRate: 80, delta: 60, rated: true },
        { hero: "Mercy", bannedPlayed: 5, bannedWon: 4, bannedWinRate: 80, notBannedPlayed: 5, notBannedWon: 1, notBannedWinRate: 20, delta: -60, rated: true },
      ],
    });
    expect(recs.some((r) => r.kind === "map_pick" && r.subject === "Nepal")).toBe(true);
    expect(recs.some((r) => r.kind === "map_avoid" && r.subject === "Ilios")).toBe(true);
    expect(recs.some((r) => r.kind === "ban_hero" && r.subject === "Sombra")).toBe(true);
    expect(recs.some((r) => r.kind === "do_not_ban_hero" && r.subject === "Mercy")).toBe(true);
