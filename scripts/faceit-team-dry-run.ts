...
} from "../src/data/faceit/team-service";
...
async function main() {
  ...
  const profile = await ScriptRuntime.runPromise(
    FaceitTeamScoutingService.pipe(
      Effect.flatMap((s) => s.getFaceitTeamProfile(teamId))
    )
  );
  if (!profile) {
  ...
  console.log(
    "Top maps:",
    JSON.stringify(profile.mapAnalysis.byMap.slice(0, 6))
  );
  console.log(
    "Attack/Defense:",
    JSON.stringify(profile.mapAnalysis.attackDefense)
  );
  console.log(
    "Hero-ban env (top 6 by |delta|):",
    JSON.stringify(profile.heroBanEnvironment.slice(0, 6))
  );
  console.log(
    "Roster:",
    JSON.stringify(
      profile.roster.map((p) => ({
        ...
        fsr: p.fsr,
        tsr: p.tsr,
        share: Number(p.appearanceShare.toFixed(2)),
      }))
    )
  );
  ...
  console.log(
    "Recommendations:",
    JSON.stringify(profile.recommendations, null, 2)
  );
}
