...
} from "../src/data/faceit/player-service";
...
async function main() {
  ...
  const players = await rt.runPromise(
    FaceitPlayerScoutingService.pipe(
      Effect.flatMap((s) => s.getFaceitPlayers())
    )
  );
  console.log(`Total players: ${players.length}`);
  const ratedId =
    process.argv[2] ?? players.find((p) => p.topFsr != null)?.faceitPlayerId;
  ...
  for (const [label, id] of [
    ...
    const p = await rt.runPromise(
      FaceitPlayerScoutingService.pipe(
        Effect.flatMap((s) => s.getFaceitPlayerProfile(id))
      )
    );
    console.log(
    ...
    console.log(
      ...
      JSON.stringify(
        p.fsrRoles.map((r) => ({
          ...
          fsr: r.fsr,
          primary: r.primary,
          headlineTier: r.headlineTier,
          tiers: r.tiers.map((t) => ({
            tier: t.tier,
            fsr: t.fsr,
            pct: Math.round(t.percentile),
          })),
          strengths: r.strengths.map((s) => s.stat),
          weaknesses: r.weaknesses.map((s) => s.stat),
        }))
      )
    );
    ...
    console.log(
      "Match history (first 3):",
      JSON.stringify(p.matchHistory.slice(0, 3))
    );
    console.log(
      "History length:",
      p.matchHistory.length,
      "Teams:",
      JSON.stringify(p.teams.slice(0, 5))
    );
  }
  await rt.dispose();
}
