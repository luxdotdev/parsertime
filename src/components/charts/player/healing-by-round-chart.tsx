...
type Props = {
  playerHealingByRound: HealingByRound[];
  playerName: string;
  playerTeam: "Team1" | "Team2";
};

export function HealingByRoundChart({
  playerHealingByRound,
  playerName,
  playerTeam,
}: Props) {
  const data: Data = playerHealingByRound.map((round, index) => ({
    name: `Round ${index + 1}`,
    playerHealing: roundNum(round.healing_dealt ?? 0),
  }));
