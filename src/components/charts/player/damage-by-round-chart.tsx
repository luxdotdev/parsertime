...
type Props = {
  playerDamageByRound: DamageByRound[];
  playerName: string;
  playerTeam: "Team1" | "Team2";
};
...
export function DamageByRoundChart({
  ...
  const data: Data = playerDamageByRound.map((round, index) => ({
    name: `Round ${index + 1}`,
    playerDamage: roundNum(round.hero_damage_dealt ?? 0),
  }));
