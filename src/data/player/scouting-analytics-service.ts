...
)<ScoutingAnalyticsService, ScoutingAnalyticsServiceInterface>() {}

const CACHE_TTL = Duration.seconds(30);
const CACHE_CAPACITY = 64;

export const make: Effect.Effect<ScoutingAnalyticsServiceInterface> =
  Effect.gen(function* () {
    function getPlayerScoutingAnalytics(
      playerName: string
    ): Effect.Effect<PlayerScoutingAnalytics, ScoutingAnalyticsQueryError> {
      const startTime = Date.now();
      const wideEvent: Record<string, unknown> = { playerName };
      ...
      return Effect.gen(function* () {
        ...
        wideEvent.has_scrim_data = true;
        wideEvent.maps_played = mapDataIds.length;
        wideEvent.hero_count = heroes.length;
        wideEvent.outcome = "success";
        yield* Metric.increment(scoutingAnalyticsQuerySuccessTotal);

        return analytics;
      }).pipe(
        Effect.tapError((error: ScoutingAnalyticsQueryError) =>
          Effect.sync(() => {
            wideEvent.outcome = "error";
            wideEvent.error_tag = error._tag;
            wideEvent.error_message = error.message;
          }).pipe(
            Effect.andThen(Metric.increment(scoutingAnalyticsQueryErrorTotal))
          )
        ),
        Effect.ensuring(
          Effect.suspend(() => {
            ...
            const log =
              wideEvent.outcome === "error"
                ? Effect.logError("player.getPlayerScoutingAnalytics")
                : Effect.logInfo("player.getPlayerScoutingAnalytics");
            return log.pipe(
              Effect.annotateLogs(wideEvent),
              Effect.andThen(
                scoutingAnalyticsQueryDuration(Effect.succeed(durationMs))
              )
            );
          })
        ),
        Effect.withSpan("player.getPlayerScoutingAnalytics")
      );
    }
    ...
    const scoutingAnalyticsCache = yield* Cache.make({
      ...
      lookup: (playerName: string) =>
        getPlayerScoutingAnalytics(playerName).pipe(
          Effect.tap(() => Metric.increment(playerCacheMissTotal))
        ),
    });

    return {
      getPlayerScoutingAnalytics: (playerName: string) =>
...
export const ScoutingAnalyticsServiceLive = Layer.effect(
