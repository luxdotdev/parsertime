...
> = Effect.gen(function* () {
  ...
  function getTeamPositionalStats(
    ...
    return Effect.gen(function* () {
      ...
      // Restrict to the team's roster so the breakdown and aggregates cover
      // active team players, not every opponent who appeared across the
      // window. getTeamRoster is the identity roster; we tag-then-filter the
      ...
      if (rosterRows.length === 0) {
        ...
        wideEvent.roster_row_count = 0;
        wideEvent.outcome = "success";
        wideEvent.early_return = "no_roster_rows";
        yield* Metric.increment(positionalStatsQuerySuccessTotal);
        return null;
      }

      const byPlayer = aggregateStatsByPlayer(rosterRows);
      ...
      wideEvent.row_count = rows.length;
      wideEvent.roster_size = roster.length;
      wideEvent.roster_row_count = rosterRows.length;
