...
import {
  ...
  scrimOverviewDuration,
  scrimOverviewErrorTotal,
  scrimOverviewSuccessTotal,
} from "./metrics";
import type {
  FightInitiatingUlt,
  MapResult,
...
export type { PlayerUltComparison, SubroleUltTiming } from "./ult-helpers";

export type ScrimOverviewServiceInterface = {
  readonly getScrimOverview: (
    scrimId: number,
    teamId: number
...
)<ScrimOverviewService, ScrimOverviewServiceInterface>() {}
...
export const make: Effect.Effect<
  ScrimOverviewServiceInterface,
  never,
  TeamSharedDataService | ScrimAbilityTimingService
> = Effect.gen(function* () {
  const teamSharedData = yield* TeamSharedDataService;
  const scrimAbilityTiming = yield* ScrimAbilityTimingService;
    ...
    return Effect.gen(function* () {
      ...
      const maps = yield* Effect.tryPromise({
        try: () =>
          ...
          new ScrimQueryError({
            operation: "getScrimOverview.fetchMaps",
            cause: error,
          }),
      }).pipe(
        Effect.withSpan("scrim.overview.fetchMaps", {
          attributes: { scrimId },
        })
      );
      ...
      if (maps.length === 0) {
        ...
        yield* Metric.increment(scrimOverviewSuccessTotal);
      ...
      wideEvent.map_data_count = mapDataIds.length;

      const teamRoster = yield* teamSharedData.getTeamRoster(teamId).pipe(
        Effect.mapError(
          (error) =>
            new ScrimQueryError({
              operation: "getScrimOverview.fetchRoster",
              cause: error,
            })
        )
      );

      const scrimPlayerNames = yield* Effect.tryPromise({
        try: () =>
          ...
          new ScrimQueryError({
            operation: "getScrimOverview.fetchPlayers",
            cause: error,
          }),
      }).pipe(
        Effect.withSpan("scrim.overview.fetchRosterAndPlayers", {
          attributes: { scrimId, teamId },
        })
      );
      ...
      if (scrimPlayers.length === 0) {
        wideEvent.outcome = "success";
        wideEvent.early_return = "no_scrim_players";
        yield* Metric.increment(scrimOverviewSuccessTotal);
        return emptyOverviewData();
      }

      wideEvent.scrim_player_count = scrimPlayers.length;
      ...
      const [
        ...
        try: () =>
          ...
          new ScrimQueryError({
            operation: "getScrimOverview.fetchAllData",
            cause: error,
          }),
      }).pipe(
        Effect.withSpan("scrim.overview.fetchAllData", {
          attributes: { scrimId, teamId, mapCount: mapDataIds.length },
        })
      );

      if (finalRoundStats.length === 0) {
        wideEvent.outcome = "success";
        wideEvent.early_return = "no_final_round_stats";
        yield* Metric.increment(scrimOverviewSuccessTotal);
        return emptyOverviewData();
      }
      ...
      for (const playerName of scrimPlayers) {
        ...
        for (let i = 0; i < maps.length; i++) {
          ...
          const mapFirstDeaths =
            fdLookup?.overallFirstDeaths.get(playerName) ?? 0;
          ...
          perMapPerformance.push({
            ...
            teamFirstDeathRate:
              mapFightCount > 0
                ? (mapTeamFirstDeaths / mapFightCount) * 100
                : 0,
          });
        }
      ...
      const baselineMap = yield* Effect.tryPromise({
        try: () =>
          getBatchedStatDistributionBaselines(Array.from(uniqueHeroes)),
        catch: (error) =>
          new ScrimQueryError({
            operation: "getScrimOverview.fetchBaselines",
            cause: error,
          }),
      }).pipe(
        Effect.withSpan("scrim.overview.fetchBaselines", {
          attributes: { heroCount: uniqueHeroes.size },
        })
      );

      const teamPlayers: PlayerScrimPerformance[] = basePlayers.map(
        (player) => {
          ...
          return { ...player, zScores, outliers };
        }
      );
      ...
      const abilityTimingAnalysis = yield* scrimAbilityTiming
        .getScrimAbilityTiming(scrimId, teamId)
        .pipe(
          Effect.mapError(
            (error) =>
              new ScrimQueryError({
                operation: "getScrimOverview.fetchAbilityTiming",
                cause: error,
              })
          ),
          Effect.withSpan("scrim.overview.fetchAbilityTiming", {
            attributes: { scrimId, teamId },
          })
        );
      ...
      wideEvent.draws = draws;
      wideEvent.team_player_count = teamPlayers.length;
      wideEvent.insight_count = insights.length;
      wideEvent.outcome = "success";
      yield* Metric.increment(scrimOverviewSuccessTotal);
      ...
      Effect.tapError((error) =>
        ...
        }).pipe(Effect.andThen(Metric.increment(scrimOverviewErrorTotal)))
      ),
      Effect.ensuring(
        Effect.suspend(() => {
          ...
          const log =
            wideEvent.outcome === "error"
              ? Effect.logError("scrim.getScrimOverview")
              : Effect.logInfo("scrim.getScrimOverview");
          return log.pipe(
            Effect.annotateLogs(wideEvent),
            Effect.andThen(scrimOverviewDuration(Effect.succeed(durationMs)))
          );
        })
      ),
      Effect.withSpan("scrim.getScrimOverview")
    );
  }
  ...
  const overviewCache = yield* Cache.make({
    ...
    lookup: (key: string) => {
      ...
      return getScrimOverview(scrimId, teamId).pipe(
  ...
  return {
    getScrimOverview: (scrimId: number, teamId: number) =>
      overviewCache
        .get(overviewCacheKeyOf(scrimId, teamId))
        .pipe(Effect.tap(() => Metric.increment(scrimCacheRequestTotal))),
  } satisfies ScrimOverviewServiceInterface;
});

export const ScrimOverviewServiceLive = Layer.effect(
  ScrimOverviewService,
  make
).pipe(
  Layer.provide(TeamSharedDataServiceLive),
  Layer.provide(ScrimAbilityTimingServiceLive),
