...
export type FaceitTeamScoutingServiceInterface = {
  readonly getFaceitTeams: () => Effect.Effect<
    FaceitTeamListEntry[],
...
export const make: Effect.Effect<FaceitTeamScoutingServiceInterface> =
  Effect.gen(function* () {
        ...
        Effect.ensuring(
          Effect.suspend(() => {
            ...
            return log.pipe(
              ...
              Effect.andThen(
                faceitTeamsQueryDuration(Effect.succeed(durationMs))
              )
            );
          })
        ),
    ...
    function getFaceitTeamProfile(
      ...
      return Effect.gen(function* () {
        ...
        const mapAnalysis = {
          byMap,
          byType,
          attackDefense: attackDefenseSplit(mapRows),
        };
        ...
        Effect.tapError((error) =>
          Effect.sync(() => {
            ...
            Effect.andThen(Metric.increment(faceitTeamProfileQueryErrorTotal))
          )
        ),
        Effect.ensuring(
          Effect.suspend(() => {
            ...
            return log.pipe(
              Effect.annotateLogs(wideEvent),
              Effect.andThen(
                faceitTeamProfileQueryDuration(Effect.succeed(durationMs))
              )
            );
          })
        ),
      ...
      return Effect.tryPromise({
        ...
        Effect.map(({ matchRowsRaw, mapRowsRaw, rosterRaw }) => {
          const matchRows: FaceitTeamMatchRow[] = matchRowsRaw.map((r) => ({
            ...
            tier: r.tier as FaceitTeamMatchRow["tier"],
            won: r.won,
          }));
          const mapRows: FaceitTeamMapRow[] = mapRowsRaw.map((r) => ({
            matchId: r.match_id,
            finishedAt: r.finished_at,
            tier: r.tier as FaceitTeamMapRow["tier"],
    ...
    function fetchBalancePatches(): Effect.Effect<
      OverwatchPatchLite[],
      FaceitScoutingQueryError
    > {
      return Effect.tryPromise({
    ...
    function fetchCorePlayers(
      teamId: string
    ): Effect.Effect<string[], FaceitScoutingQueryError> {
      return Effect.tryPromise({
      ...
      `,
          ...
          new FaceitScoutingQueryError({
            operation: "fetch core players",
            cause: error,
          }),
      }).pipe(Effect.map((rows) => rows.map((r) => r.pid)));
    }

    function fetchRelatedTeams(
      teamId: string
    ): Effect.Effect<RelatedTeam[], FaceitScoutingQueryError> {
      return Effect.tryPromise({
        try: () => prisma.$queryRaw<
          {
            faceit_team_id: string;
      ...
      `,
        ...
        Effect.map((rows) =>
          rows.map((r) => ({
            ...
            name: r.name,
            matchCount: Number(r.match_count),
            sharedCorePlayers: Number(r.shared),
          }))
        )
      );
    }
...
export const FaceitTeamScoutingServiceLive = Layer.effect(
