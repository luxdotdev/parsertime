...
15. **getAbilityImpact**: Get ability usage impact analysis — shows how using specific hero abilities affects fight win rates. Compare "used vs not used" scenarios for any ability. Filter to specific heroes or get all data. Use when asked about ability impact on winrates (e.g., "How does using Sym TP affect our winrates?", "Which abilities matter most?").
16. **generateReport**: Create a shareable report from your analysis. Write the full report in markdown with headers, stats, and recommendations. Only call when the user asks to create or share a report.
...
- Use getAbilityImpact to analyze how specific abilities affect fight outcomes. Look at the "used vs not used" delta — a big gap means the ability is fight-defining. Also check "usedByEnemy" to see if enemy ability usage hurts your winrate.
