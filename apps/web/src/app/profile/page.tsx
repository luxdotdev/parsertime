...
// Explicit return type: the body always redirects/404s, so TS otherwise
// infers Promise<never>, which is not a valid JSX component type.
async function ProfileRedirect(): Promise<React.ReactNode> {
...
// This page only redirects to /profile/[battletag], so the fallback mirrors
// the destination profile page's frame — the redirect lands without a
// design swap.
  ...
  return (
    <div className="px-6 pt-8 pb-16 sm:px-10">
      <header className="border-border flex flex-wrap items-end justify-between gap-x-10 gap-y-4 border-b pb-8">
        ...
        <div className="flex flex-wrap items-end gap-x-8 gap-y-2 pb-1">
          {["a", "b", "c", "d"].map((k) => (
      ...
      <div className="border-border mt-6 flex gap-6 border-b pb-3">
        {["a", "b", "c", "d"].map((k) => (
          <Skeleton key={k} className="h-3 w-16" />
        ))}
      </div>
        ...
        <section className="space-y-4">
          ...
          <ul className="bg-border grid grid-cols-1 gap-px">
            {["a", "b", "c", "d", "e", "f", "g", "h"].map((k) => (
              <li key={k} className="bg-card flex items-center gap-4 px-5 py-3">
        ...
        <section className="space-y-8">
          ...
          {["a", "b", "c"].map((k) => (
            <div key={k} className="space-y-3">
              ...
              <div className="bg-border grid grid-cols-1 gap-px sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {["a", "b", "c", "d"].map((j) => (
