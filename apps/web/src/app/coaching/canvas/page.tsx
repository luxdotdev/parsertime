...
// Static shell: the page frame and header prerender (default-locale text via
// the cookie-free translator); the flag-gated canvas streams into ONE boundary
// whose fallback mirrors CoachingCanvas's own layout.
export default function CoachingCanvasPage() {
  const t = getStaticTranslations("coaching");
...
async function CoachingCanvasContent() {
  const enabled = await getFlag(coachingCanvas);
  if (!enabled) notFound();

  return <CoachingCanvas />;
}

// Mirrors CoachingCanvas's layout (toolbar row, 20% hero sidebar, canvas
// panel) so the streamed canvas replaces this in place with no jump.
