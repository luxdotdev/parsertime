...
// Static shell: only the page frame prerenders. The header eyebrow depends on
// the ?hero search param, so the header streams in with the content — one
// boundary, one fallback that mirrors the loaded layout.
export default function CsrLeaderboardPage(props: {
  searchParams: Promise<{ hero?: string }>;
}) {
  return (
    <div className="px-6 pt-8 pb-16 sm:px-10">
      <Suspense fallback={<CsrLeaderboardSkeleton />}>
        <CsrLeaderboardContent searchParams={props.searchParams} />
      </Suspense>
    </div>
  );
}

async function CsrLeaderboardContent(props: {
  searchParams: Promise<{ hero?: string }>;
}) {
...
// Mirrors the loaded layout: header block, subnav + hero-selector row, then
// the LeaderboardWithStats grid (its 7fr/5fr table + stats aside).
function CsrLeaderboardSkeleton() {
  return (
    <>
      <header className="border-border flex flex-wrap items-end justify-between gap-x-10 gap-y-4 border-b pb-6">
        <div>
